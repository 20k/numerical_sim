...
float buffer_read_nearest_clamph(__global const half* const buffer, int3 position, int4 dim)
{
    position = clamp(position, (int3)(0,0,0), dim.xyz - 1);

    return vload_half(position.z * dim.x * dim.y + position.y * dim.x + position.x, buffer);
}

float buffer_read_linearh(__global const half* const buffer, float3 position, int4 dim)
{
    float3 floored = floor(position);
