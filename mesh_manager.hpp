...
struct evolution_points
{
    ...
    evolution_points(cl::context& ctx) : first_derivative_points(ctx), second_derivative_points(ctx), border_points(ctx), all_points(ctx), order(ctx){}
};
